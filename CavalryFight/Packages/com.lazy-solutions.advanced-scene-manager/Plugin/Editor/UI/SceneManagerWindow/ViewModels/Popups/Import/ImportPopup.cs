...
namespace AdvancedSceneManager.Editor.UI.Views.Popups
{
    ...
    abstract class ImportPopup<T, TSelf> : ViewModel, IPopup where TSelf : ViewModel, new()
    {

        public override VisualTreeAsset template => ((SceneManagerWindow)window).viewLocator.popups.importScene;
        ...
        protected override void OnAdded()
        {
            ...
            RegisterEvent<OnWindowEnableEvent>(e => Reload());
        ...
        readonly List<EventCallback<IEnumerable<T>>> callbacks = new();

        public void RegisterItemCheckedCallback(EventCallback<IEnumerable<T>> callback) => callbacks.Add(callback);
        public void UnregisterItemCheckedCallback(EventCallback<IEnumerable<T>> callback) => callbacks.Remove(callback);

        void InvokeCallbacks()
        {
            foreach (var callback in callbacks)
            {
                callback.Invoke(items.Where(i => i.isChecked).Select(i => i.value));
            }
        }
        ...
        void SetupFooterButton(ref Button button, string name, string text, Action click)
        {
            ...
            button.clicked += () =>
            {
                ...
                GetService<IPersistentNotificationService>().ReloadNotifications();
            };
