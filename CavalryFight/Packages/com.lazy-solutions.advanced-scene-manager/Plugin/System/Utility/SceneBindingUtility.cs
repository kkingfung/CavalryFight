...
namespace AdvancedSceneManager.Utility
{
    ...
    public static class SceneBindingUtility
    {
        ...
        static void SetupTracking()
        {
            ...
            {
                m_standaloneScenes.Remove(e.scene);
                Persist();
            });

            SceneManager.events.RegisterCallback<CollectionCloseEvent>(e =>
            {
                if (e.collection == m_openCollection)
                {
        ...
        static void RestoreTrackedItems()
        {
            ...
            EditorApplication.delayCall += () =>
            {
                ...
                var collection = SceneCollection.Find(EditorPrefs.GetString("ASM.SceneBindings.Collection"));
        ...
        public static IEnumerable<(SceneCollection collection, Scene scene, Models.InputBinding binding)> GetBindings()
        {
            ...
            var collections = SceneManager.profile.collections.Where(c => c && (c.inputBindings.Any(b => b.isValid)));
            var scenes = SceneManager.profile.standaloneScenes.NonNull().Where(s => s.inputBindings.Any(b => b.isValid));
        ...
        static void OnPress(SceneCollection collection, Models.InputBinding binding)
        {
            ...
            if (!collection.isOpen && CheckPreload())
            {
                ...
                if (binding.openCollectionAsAdditive)
                    SpamCheck.EventMethods.Execute(() => collection.OpenAdditive());
                else
                    SpamCheck.EventMethods.Execute(() => collection.Open());
            }
            else if (collection.isOpen && binding.interactionType == InputBindingInteractionType.Toggle && CheckPreload())
                SpamCheck.EventMethods.Execute(() => collection.Close());
        ...
        static void OnPress(Scene scene, Models.InputBinding binding)
        {
            ...
            {
                Track(scene);
                SpamCheck.EventMethods.Execute(() => scene.OpenWithLoadingScreen(scene.inputBindingsLoadingScene).IsStandaloneScene());
            }
            else if (scene.isOpen && binding.interactionType == InputBindingInteractionType.Toggle && CheckPreload())
                SpamCheck.EventMethods.Execute(() => scene.CloseWithLoadingScreen(scene.inputBindingsLoadingScene).IsStandaloneScene());
