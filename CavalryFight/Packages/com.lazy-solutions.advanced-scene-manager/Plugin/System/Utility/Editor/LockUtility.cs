...
namespace AdvancedSceneManager.Editor.Utility
{
    ...
    public static partial class LockUtility
    {
        ...
        public static void Lock(this ILockable obj, string message = null, bool prompt = false)
        {
            ...
            if (prompt && !PromptUtility.PromptString($"Locking object...", "Lock reason:", out message, message))
                return;
        ...
        public static void Unlock(this ILockable obj, bool prompt = false)
        {
            ...
            if (prompt && !PromptUtility.Prompt($"Unlocking object...", string.IsNullOrWhiteSpace(obj.lockMessage) ? "No message" : obj.lockMessage))
                return;
        ...
        [ASMWindowElement(ElementLocation.Collection, isVisibleByDefault: true)]
        private static VisualElement CollectionButton(ViewModelContext context)
        {

            if (!context.collection)
                return null;

            var button = new Button(() => context.collection.Unlock(prompt: true)) { text = "" };
            button.UseFontAwesome();

            ReloadVisibility();

            //Property changed
            context.collection.PropertyChanged += Collection_PropertyChanged;
            SceneManager.settings.project.PropertyChanged += Project_PropertyChanged;

            //Unregister
            context.collection.onDestroy += Collection_onDestroy;
            button.RegisterCallbackOnce<DetachFromPanelEvent>(e =>
            {
                context.collection.PropertyChanged -= Collection_PropertyChanged;
                context.collection.onDestroy -= Collection_onDestroy;
                SceneManager.settings.project.PropertyChanged -= Project_PropertyChanged;
            ...
            {
                var isLocked = SceneManager.settings.project.allowCollectionLocking && context.collection.isLocked;
                button.visible = isLocked;
                button.style.position = isLocked ? Position.Relative : Position.Absolute;
            }
            ...
            void Collection_PropertyChanged(object sender, PropertyChangedEventArgs e) => ReloadVisibility();
            void Project_PropertyChanged(object sender, PropertyChangedEventArgs e) => ReloadVisibility();
