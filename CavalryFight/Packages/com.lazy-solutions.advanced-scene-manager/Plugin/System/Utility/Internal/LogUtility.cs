    ...
    static class LogUtility
    {
...
#if UNITY_EDITOR
            if (SceneManager.settings.user.logStartup)
                Log(StartupCompleteMessage());
#endif
        }

        static void Log(string message)
        {
            AdvancedSceneManager.Log.Info(message, onlyLogInDev: false);
        }
...
#if UNITY_EDITOR
            if (SceneManager.settings.user.logTracking)
                Log(TrackedMessage(collection, isAdditive));
#endif
        }
...
#if UNITY_EDITOR
            if (SceneManager.settings.user.logTracking)
                Log(UntrackedMessage(collection, isAdditive));
#endif
        }
        ...
        public static void LogImported(IEnumerable<Scene> list)
        {
            if (SceneManager.settings.user.logImport)
                Log($"Imported {list.Count()} scenes:\n{string.Join("\n", list.Select(s => s.path))}\n");
        }
        ...
        {
            if (SceneManager.settings.user.logImport)
                Log($"Unimported {list.Count()} scenes:\n{string.Join("\n", list.Select(s => s.path))}\n");
        }

        public static void LogUnimported(string path)
        {
            if (SceneManager.settings.user.logImport)
                Log($"Unimported 1 scenes:\n{path}\n");
        }
