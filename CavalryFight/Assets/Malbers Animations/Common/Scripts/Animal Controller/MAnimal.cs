...
#pragma warning restore 414
        ...
        void Reset()
        {
            MTools.SetLayer(t, 20);     //Set all the Children to Animal Layer   .
            ...
            if (MainCollider != null)
            {
                ...
                if (Editor_Tabs1 == 0)
                {
                    ...
                    foreach (var col in colliders)
                    {
                        ...
                        {
                            MDebug.GizmoCapsule(capsule.transform.TransformPoint(capsule.center), capsule.transform.rotation, capsule.height, capsule.radius, Color.green, capsule.direction, 16);
                        }
                        else if (col is BoxCollider box)
                        {
                            Gizmos.matrix = col.transform.localToWorldMatrix;
                            Gizmos.DrawWireCube(box.center, box.size * sc);
                        }
                        else if (col is SphereCollider sphere)
                        {
                            Gizmos.matrix = col.transform.localToWorldMatrix;
                            Gizmos.DrawWireSphere(sphere.center, sphere.radius * sc);
                        }
                    }
