...
namespace BlazeAISpace
{
    ...
    public class CoverShooterBehaviour : BlazeBehaviour
    {
        ...
        public override void Close()
        {
            ...
            if (blaze.state != BlazeAI.State.goingToCover && blaze.state != BlazeAI.State.attack) {
        ...
        public override void Main()
        {
            ...
            blaze.isAttackingAnim = singleShotPass;
        ...
        bool CanSeeTarget(bool onAttacking = false)
        {
            ...
            Vector3 start = transform.position + blaze.vision.visionPosition;
            Vector3 targetPoint = enemyCollider.ClosestPoint(start);
            ...
            int hits = Physics.SphereCastNonAlloc(start, 0.05f, dir, canSeeTargetHitArr, dist, layers);
            ...
            {
                RaycastHit hit = canSeeTargetHitArr[i];
            ...
            if (gtcBehaviour.lastCover != null) 
            {
                if (item != null) 
                {
                    if (gtcBehaviour.lastCover.root == canSeeTargetHitArr[bestIndex].collider.transform.root)
                    {
                        return false;
                    }
                }
            ...
            if (onAttacking)
            {
                if (item != null && layersCheckOnAttacking != 0)
                {
                    if (!canSeeTargetHitArr[bestIndex].collider.transform.root == blaze.enemyToAttack.transform.root && (layersCheckOnAttacking & (1 << canSeeTargetHitArr[bestIndex].transform.gameObject.layer)) != 0)
                    {
        ...
        public virtual void ReachableBehaviour(float minDistance, float backupDistTemp, bool foundNoCoverPass)
        {
            ...
            if (blaze.distanceToEnemySqrMag > (backupDistTemp * backupDistTemp))
            {
                ...
                if (blaze.isAttacking)
                {
                    ...
                    if (enemySeenFrames > 0)
                    {
                        ...
                        else Attack(false, canMoveForward);
                    }
                    else {
                        MoveToTarget(blaze.enemyColPoint);
                    }
            ...
            if (blaze.CheckIfTargetSeenFromPoint(transform.position - (transform.forward * (blaze.navmeshAgent.height - 0.25f))))
            {
        ...
        void Strafe()
        {
            ...
            // if strafing then continue with strafe movement
            if (isStrafing)
            {
                if (changeStrafeDir)
                {
        ...
        void SetSearchPoint()
        {
            float radiusDistance = (blaze.navmeshAgent.height * 2);
            blaze.ClosestNavMeshPoint(blaze.RandomSpherePoint(blaze.enemyColPoint, radiusDistance, false), 5, out searchPointLocation);
