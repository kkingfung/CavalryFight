...
namespace BlazeAISpace
{
    ...
    public class Waypoints
    {
        ...
        [Tooltip("Setting this to true will make the AI loop the waypoints when patrolling, setting it to false will stop at the last waypoint.")]
        ...
        [Tooltip("Enabling randomize will instead generate randomized waypoints within a radius from the start position in a continuous fashion and won't use the pre-set waypoints.")]
        public bool randomize = true;
        [Min(0), Tooltip("The radius from the start position to get a randomized position.")]
        public float randomizeRadius = 20f;
        [Tooltip("This ensures that when the AI generates a random point, it's on the same level or with a slight difference as set here. Set the min and max level difference.")]
        public Vector2 minAndMaxLevelDiff = new Vector2(-0.5f, 0.5f);
        [Tooltip("Shows the radius as a yellow disc in the scene view. Randomize needs to be enabled for this to show.")]
        ...
        public void Draw(Vector3 position, BlazeAI blaze)
        {
            if (randomize) 
            {
                if (showRandomizeRadius) 
                {
                    ...
                    RaycastHit hit;
                    if (Physics.Raycast(blaze.transform.position, -Vector3.up, out hit, Mathf.Infinity, Physics.AllLayers)) {
                        Debug.DrawRay(blaze.transform.position, hit.point - blaze.transform.position, Color.yellow, 0.1f);
                        UnityEditor.Handles.color = Color.yellow;
                        UnityEditor.Handles.DrawWireDisc(hit.point, blaze.transform.up, randomizeRadius);
                    }
                }
